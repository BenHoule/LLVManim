...
class CFGTraversal(Scene):
    ...
    def _traverse_edge(self, src: str, dst: str) -> None:
        ...
        if getattr(group, "is_dashed", False) and key not in self._traversed:
            dashed_mob = group[0]
            solid: VMobject = group.solid_curve  # type: ignore[attr-defined]
            solid.set_color(_EDGE_ACTIVE).set_stroke(width=3.5)
            tip: VMobject = group.arrow_tip  # type: ignore[attr-defined]
            ...
            self.play(
                ...
                group.arrow_tip.animate.set_fill(color=_EDGE_ACTIVE),  # type: ignore[union-attr]
                run_time=0.2,
            )

        curve = group[0]
        tip_mob: VMobject = group.arrow_tip  # type: ignore[attr-defined]
        self.play(
            curve.animate.set_color(_EDGE_TRAVERSED).set_stroke(width=3),
            tip_mob.animate.set_fill(color=_EDGE_TRAVERSED),
            run_time=0.15,
        )
